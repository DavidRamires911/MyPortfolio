import { useContext } from "react";
import { ThemeContext } from "../ThemeContext";

const ProjectItem = ({ img, title, Description }) => {
  const { darkMode } = useContext(ThemeContext);

  const hover = darkMode
    ? " shadow-orange-400 from-orange-200 to-gray-800 "
    : " shadow-gray-400 from-gray-200 to-gray-800  ";

  return (
    <div
      className={`relative flex items-center justify-center h-auto w-full shadow-xl mt-8 rounded-xl group hover:bg-gradient-to-r ${hover} }`}
    >
      <img
        src={img}
        alt={title}
        className="rounded-xl group-hover:opacity-10 "
        style={{ width: "auto", height: "320px" }}
      />
      <div className="hidden group-hover:block absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] ">
        <h3 className="text-2xl font-bold text-white tracking-wider text-center ">
          {" "}
          {title}{" "}
        </h3>
        <p className="pb-4 pt-2 text-white text-center "> {Description} </p>

        <p className="text-center  p-3 rounded-lg bg-white text-gray-700 font-bold cursor-pointer text-lg ">
          {" "}
          Website{" "}
        </p>
      </div>
    </div>
  );
};

export default ProjectItem;
